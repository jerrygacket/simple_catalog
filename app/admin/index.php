...
//        echo '<pre>';print_r($filter_conditions);echo '</pre>';die();
        if (!empty($filter_conditions)) {
            $where_conditions[] = '(' . implode(' or ', $filter_conditions) . ')';
            $having_conditions[] = 'COUNT(DISTINCT opt.name) = ' . count($filter_conditions);
        }
    }
...
extract(getProducts($pdo, $filters));
?>
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
<pre>
        ...
        <?php foreach ($options as $option): ?>
            <fieldset>
                ...
                <?php elseif ($option['type'] === 'multiple'): ?>
                    <?php foreach ($option['values'] as $value): ?>
                        <label>
                            <input type="checkbox" name="<?php echo $option['name']; ?>[]" value="<?php echo $value['id']; ?>" <?= in_array($value['id'], $filters[$option['name']] ?? []) ? 'checked=""' : ''?>>
                            <?php echo htmlspecialchars($value['value']); ?>
                        </label>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <p>Showed: <strong><?php echo count($products); ?> of <?php echo $total; ?> products</strong></p>
